...
var shit = [["FINISHED","WIP","ALL"],[0xFF8C81D9,0xFFC63C3f,0xFFDCF5F4],[[866,433],[866,0],[0,-430]]];
function create() {
	...
	var text:FlxText = new FlxText(textBG.x, textBG.y + 4, FlxG.width, 'Press TAB to switch mods / Press O for options / Press SEVEN for editor picker.');
...
function update(elapsed:Float) {time += elapsed;
	...
	if (controls.SWITCHMOD||FlxG.keys.justPressed.SEVEN) {
		...
		controls.SWITCHMOD ? openSubState(new ModSubState('global/ModSwitchRon')) :openSubState(new EditorPicker());
