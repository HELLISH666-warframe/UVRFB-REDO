...
var t = Paths.getSparrowAtlas("menus/V3/win/run tab");
...
function create() {
	...
	var iconI:Int = 0;
	var iconFrames = Paths.getFrames("menus/V3/win/menuIcons");
	...
	add(sanstitre = new FlxBackdrop(Paths.image('menus/V3/win/sanstitre'), FlxAxes.XY, 0, 0));
	add(rainbowscreen = new FlxBackdrop(Paths.image('menus/V3/win/rainbowpcBg'), FlxAxes.XY, 0, 0));
	add(new FlxSprite().loadGraphic(Paths.image("menus/V3/win/pcBg")));
	if (FlxG.save.data.crt)FlxG.camera.addShader(crt = new CustomShader("V3/fake CRT"));
	if (FlxG.save.data.colour) {FlxG.camera.addShader(bit = new CustomShader("V3/8bitcolor"));
	bit.enablethisbitch = 1.;}
	if (FlxG.save.data.chrom) FlxG.camera.addShader(chrom);
	
	add(window = CoolUtil.loadAnimatedGraphic(new FlxSprite(FlxG.width/1.3-405,ywindow),Paths.image('menus/V3/win/menuCarNew'))).angle = 3;
		...
		button = new FlxButton((iconI > 2 ? 180 : 20), 20 + (150 * (iconI > 2 ? iconI - 3:iconI)), "", function() {
			...
			if (curClicked == i) {
				...
				if (clickAmounts == 2) {
					...
					if (icons[i] == "story mode is idiot") {FlxG.switchState(new ModState('stupid_fucking-video_thing'));}
